...
use lb_node::config::{
    blend::deployment::MinimumNetworkSize, cryptarchia::deployment::EpochConfig,
};
...
async fn leader_claim() {
    ...
    let configs: Vec<_> = configs
        ...
        .map(|c| {
            ...
            config.deployment.blend.common.minimum_network_size =
                MinimumNetworkSize::try_new(10).unwrap();
...
async fn claim_leader_rewards(addr: SocketAddr, duration: Duration) {
    timeout(duration, async {
        loop {
            let response = reqwest::Client::new()
                .post(format!("http://{addr}/leader/claim"))
                .send()
                .await
