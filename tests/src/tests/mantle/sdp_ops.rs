...
async fn start_sdp_manual_cluster(
    ...
    let base = build_local_manual_cluster(
        test_name,
        "tf-sdp",
        DeploymentBuilder::new(TfTopologyConfig::with_node_numbers(1)).with_wallet_config(
            WalletConfig::new(vec![funding_wallet.clone(), spare_wallet.clone()]),
        ),
    );
    ...
    wait_for_manual_cluster_height(&node0.client, 1, Duration::from_secs(120))
        .await
